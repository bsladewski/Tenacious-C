...
import { executeWithEngineAdapter } from '../engines/execute-with-fallback';
 ...
 * @param toolType - CLI tool type to use for generating the summary
 * @param fallbackTools - Optional list of fallback tools to try if primary fails
...
export async function generateFinalSummary(
  ...
  // We need to capture the output, so we create a temporary file for the output
  const summaryOutputFile = resolve(timestampDirectory, 'final-summary-output.txt');

  // Modify the prompt to include explicit phase marker and output file instruction
  // The phase marker helps MockAdapter reliably detect this as generate-summary phase
...
Write ONLY the summary content to the file - no additional commentary or explanations.`;
  ...
  // Execute using engine adapter (works with mock mode)
  ...
  // Use executeWithEngineAdapter instead of deprecated AICliTool interface
  await executeWithEngineAdapter(
    toolType,
    promptWithOutput,
    null, // No model specified for summary generation
    fallbackTools,
    summaryContext
  );
