...
**CRITICAL:** The plan file and requirements file contain a "Verification Tools / Gates" section (or "Verification Requirements" section) that was auto-generated by the Tool Curation phase. These verification commands are MANDATORY.

- You MUST run all verification commands listed in those sections before this execution is considered complete
...
The plan file and requirements file may contain an "Agent Skills" section that was auto-generated by the Tool Curation phase. These files contain project-specific conventions, coding standards, and rules that the executing agent MUST follow.

- **Read the listed agent skill files** before starting implementation
- **Follow all conventions** described in these files (coding standards, naming conventions, architecture rules, testing patterns, etc.)
- If a skill file is agent-specific (e.g., Cursor rules vs. Claude instructions), prioritize the one matching the current CLI tool, but treat all listed conventions as binding
...
- [ ] Created \`{{outputDirectory}}/execution-summary-{{executionIteration}}.md\` with the execution summary
- [ ] Created \`{{outputDirectory}}/execute-metadata.json\` with ONLY the allowed keys (schemaVersion, hasFollowUps, hardBlockers, summary)
...
- [ ] The \`hasFollowUps\` boolean correctly reflects whether there are remaining follow-ups
