...
**CRITICAL:** The original plan file and requirements file contain a "Verification Tools / Gates" section (or "Verification Requirements" section) that was auto-generated by the Tool Curation phase. These verification commands remain MANDATORY during follow-up execution.
...
The plan file and requirements file may contain an "Agent Skills" section that was auto-generated by the Tool Curation phase. These files contain project-specific conventions, coding standards, and rules that remain MANDATORY during follow-up execution.

- **Follow all conventions** described in the listed agent skill files
- If your follow-up changes introduce new code, ensure it conforms to the project standards in these files
- If the plan or requirements do not contain an "Agent Skills" section, still check for common agent instruction files (e.g., \`.cursorrules\`, \`CLAUDE.md\`, \`AGENTS.md\`) in the repository root
