...
## Verification Command Status (Required Check)

**CRITICAL:** The plan file and requirements file contain a "Verification Tools / Gates" section (or "Verification Requirements" section) that was auto-generated by the Tool Curation phase.

You MUST verify:
1. Check the plan's "Verification Tools / Gates" section
2. Verify that all listed commands were run during execution
3. Verify that all commands passed (or failures are documented with justification)

**If verifications were not run or are failing without documented justification:**
- Set \`gapsIdentified\` to \`true\`
- List the verification gap in the audit summary
